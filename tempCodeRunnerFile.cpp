...
void selection_sort() {
    ...
    for (int i = 0; i < n - 1; i++) {
        int min_index = i;
        for (int j = i + 1; j < n; j++) {
            if (mahasiswa_list[j].npm < mahasiswa_list[min_index].npm) {
                min_index = j;
            }
        }
        std::swap(mahasiswa_list[i], mahasiswa_list[min_index]);
    }
...
int main() {
    while (true) {
        ...
        std::cout << "3. Urutkan Mahasiswa (Selection Sort)" << std::endl;
